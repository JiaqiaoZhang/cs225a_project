	...
	while (runloop)
	{
		...
		if (state == JOINT_CONTROLLER)
		{
			...
			if ((robot->_q - q_curr_desired).norm() < 0.05)
			{
				...
				{
					state = POSORI_CONTROLLER;
					cout << "Dropping food on grill..." << endl
							 << endl;
					task = DROP_FOOD;
				...
				else if (task == RESET)
				{
					...
					if (grill_index < 3)
					{
						...
						cout << "Current Food..." << grill_index << endl
								 << endl;
		...
		else if (state == POSORI_CONTROLLER)
		{
			...
			if (posori_task->goalPositionReached(0.01) && posori_task->goalOrientationReached(0.05))
			{
				...
				else if (task == LIFT_SPATULA)
				{
					...
					{
						state = POSORI_CONTROLLER;
						cout << "Plating food #" << plate_index << " ..." << endl
								 << endl;
						task = PLATE;
						posori_task->reInitializeTask();
				...
				else if (task == FLEX_WRIST)
				{
					...
					if (grill_index < 3)
					{
						state = JOINT_CONTROLLER;
						task = RESET;
						cout << "Moving to initial station..." << endl
								 << endl;
						joint_task->reInitializeTask();
						station = STATION_2;
					}
					...
					{
						state = POSORI_CONTROLLER;
						cout << "Aligning for plate#" << plate_index << "..." << endl
								 << endl;
						task = ALIGN;
						posori_task->reInitializeTask();
						//if(plate_index == 1){
				...
				else if (task == ALIGN)
				{
					...
					if (grill_index < 3)
					{
						cout << "Sliding for Grill Food " << grill_index << "..." << endl
								 << endl;
					}
					else if (plate_index < 3)
					{
						cout << "Sliding for Plate Food " << plate_index << "..." << endl
								 << endl;
					}
					posori_task->reInitializeTask();
					posori_task->_use_velocity_saturation_flag = true;
		...
		for (int f = 0; f < 3; f++)
		{
			...
			if (food_actuate[f] == true)
			{
				...
				if (f == 0)
				{
					...
					{
						cout << "bottom_bun_actuate = " << bottom_bun_actuate << "... bottom_bun_command_torques = " << bottom_bun_command_torques.transpose() << endl
								 << endl;
					}
				}
				...
				else if (f == 1)
				{
					...
					if (controller_counter % 10000 == 0)
					{
						cout << "burger_actuate = " << burger_actuate << "... burger_command_torques = " << burger_command_torques.transpose() << endl
								 << endl;
					}
				}
				else if (f == 2)
				{
					...
					if (controller_counter % 10000 == 0)
					{
						cout << "top_bun_actuate = " << top_bun_actuate << "... top_bun_command_torques = " << top_bun_command_torques.transpose() << endl
								 << endl;
					}
				}
			}
